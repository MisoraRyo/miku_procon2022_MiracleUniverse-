...
4. **キャラクターのアニメーション設定**： セグメントのよる分類を、各キャラクターのアニメーション演出の分岐に利用しています。この分岐を起点にユーザーの好きなアニメーションパターンに設定することを可能としています。現在は５種類のアニメーションを設定していますが、もっと多くのアニメーションを設定追加することができます。（mixamoのアニメーションデータを利用しています）
...
2. **演出**： できるだけ多くの人のPCでも動作するように、控えめな演出にとどめて設計しています。
...
スタート時の中央の八面体、もしくは左下のPLAYアイコンで楽曲の再生、左下のSTOPアイコンで停止ができます。
...
＊PCで閲覧の場合、Safariブラウザでは極端にFPSが下がる場合がございます。（Three.jsとMac_OSのバグで解決策がわかりません）ChromeまたはFirefoxを推奨します。
